...
# Constants
if len(sys.argv) != 4:
    print("Usage: python3 script.py TWITCH_URL FILE_OFFSET FILE_INDEX")
    sys.exit(1)
...
CSV_PATH = 'twitch_data'+ FILE_OFFSET + '_' + FILE_INDEX +'.csv'
HTTP_LOGS_PATH = 'http_logs'+ FILE_OFFSET + '_' + FILE_INDEX +'.json'
...
with open(CSV_PATH, 'w', newline='') as file:
    fieldnames = [
        ...
        "Backend Version", "Serving ID", "Codecs", "Play Session ID", "Protocol", "isRebuffering"
    ]
...
# Change 20 to 1800 when not experiementing
for _ in range(20):
    driver.execute_script(js_analyze)
    time.sleep(0.1)
    last_log_timestamp = process_logs(driver, last_log_timestamp)
